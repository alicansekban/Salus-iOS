...
struct RootView: View {
    ...
    /// One tab's stack, in ``RootNavigationStack``. A view of its own, in its own file: the five
    /// stacks and their destination registrars are the longest single thing the shell does, and
    /// this file has room for that or for everything else, not both. It reads `root` and
    /// `backStacks` and nothing else, so it is handed exactly those.
    private func navigationStack(for tab: RootTab) -> some View {
        RootNavigationStack(tab: tab, root: root, backStacks: backStacks)
    }
