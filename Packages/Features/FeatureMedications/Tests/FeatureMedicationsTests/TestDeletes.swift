...
// Fakes.kt:109-118`, whose `TestDeletes` is byte-identical to `:feature:appointments`' and
// `:feature:vitals`' copies. The Swift file is therefore the same copy as
// `FeatureAppointmentsTests/TestDeletes.swift`, and the feature template sanctions the duplicate: a
// test target cannot import another package's tests.
//
...
/// the app actually runs rather than on a stand-in (`Fakes.kt:109-118`).
@MainActor
final class TestDeletes {
    ...
    /// Kotlin takes a `windowMillis` (`Fakes.kt:109`); this does not. The gate below ignores
    ...
    /// Invokes the snackbar action the last delete published (`Fakes.kt:114-117`).
    func undoLast() {
