// No Android twin, copied verbatim from
// `Packages/Features/FeatureSettings/Tests/FeatureSettingsTests/WaitUntil.swift` (T4). Kotlin's
// ViewModel tests drive `runTest`'s virtual scheduler with `advanceUntilIdle()`; Swift Testing has
// no such scheduler, so the same "let every task that is already runnable finish" step is spelled
// as a bounded yield loop.
//
