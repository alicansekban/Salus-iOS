// Ported from
// `feature/appointments/src/test/kotlin/com/alicansekban/salus/feature/appointments/
// TestDeletes.kt`, which is byte-identical to `:feature:vitals`' copy. The Swift file is therefore
// the same copy as `FeatureVitalsTests/TestDeletes.swift`, and the feature template sanctions the
// duplicate: a test target cannot import another package's tests.
//
...
//     `TestDeletes.kt:11-20` exists for.
...
/// the app actually runs rather than on a stand-in (`TestDeletes.kt:26-36`).
@MainActor
final class TestDeletes {
    ...
    /// Invokes the snackbar action the last delete published (`TestDeletes.kt:32-35`).
    func undoLast() {
