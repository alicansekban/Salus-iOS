...
/// language) and `values-en/strings.xml`, and the drift detector between the two locales: all 87
...
/// with M8. `SettingsStrings.swift`'s header carries the card-by-card mapping and the reason each
...
struct SettingsStringsTests {
    ...
    @Test("the catalog holds exactly the 87 keys :feature:settings owns")
...
///
/// Held in a dedicated file-private enum so the 87-row table does not blow the suite's own body
