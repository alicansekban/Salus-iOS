...
| 2.3b | On that sheet, look at the space under the last line *(default text size, owner QA round 2 C2)* | the sheet ends just under the paragraph — it is as tall as its content, with no empty area under the last line and no need to drag it down | ☐ |
...
| 5.2.11 | Profile (avatar → Profil): scroll to the bottom and look at "Değişiklikleri Kaydet" *(owner QA round 3, D2)* | the button's pill **starts and ends where the fields above it do** — the same inset as the name field, the height field and the caption over it — and never touches either screen edge. Check it in landscape too, where a full-bleed pill is widest | ☐ |
