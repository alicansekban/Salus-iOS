...
| (z) | `SalusStepperField` draws the create-mode suggestion at reduced opacity — `onSurfaceVariant` at `SalusStepperFieldDefaults.suggestionOpacity` (0.45, above Material's 0.38 disabled floor) — and Android has since adopted the same alpha (`SalusStepperFieldDefaults.SuggestionAlpha` 0.45, `SalusStepperField.kt` decoration box, Android main `a134c0a`), so the two platforms read the empty state the same way and this row records history, not a live divergence. The weakness the owner met on iOS: at full tone the suggestion reads as a number the user typed while Save stays disabled. The committed value's colour is unchanged, and `SalusUIStrings.stepperSuggested` still says the same thing to VoiceOver (owner QA round 2 C1, `Packages/SalusUI/Sources/SalusUI/component/SalusStepperField.swift:40-47`, `:172-186`). |
...
| (bb) | `SalusSelectableRow` **is transparent in every state**, exactly as `SalusSelectableRow.kt:56-68` is: a bare `Row` with `padding(horizontal = lg, vertical = md)`, `spacedBy(md)` and `defaultMinSize(minHeight = SalusTouchTarget.min)`, no `Surface`, no background, no border — selection is the trailing radio mark alone (`primary` dot and ring when selected, an `onSurfaceVariant` ring otherwise, `:100-103`). The `surfaceVariant` pill under every row, the `primaryContainer` fill and the 2 pt `primary` stroke under the selected one, and the `outlineVariant` unselected ring that this port drew until owner QA round 3 were carried over from the pre-M16 `SalusOptionRow` the component was renamed from (§3.2) and had no line in the Kotlin behind them. What remains a divergence is the **API**: Kotlin passes the leading visual into a `leading` slot (`:52`, `:69`), iOS takes `swatch: Color?` / `systemImage: String?` + `accent:`, so the tile's size belongs to the component rather than to the caller. It is pinned at the size the only Kotlin caller passes — `SalusIconBadge(size = Small, iconSize = SmallIconSize)`, 24 / 14 (`LanguageSheet.kt:70-76`) — which is also the 24 pt box `SalusSelectableRowDefaults.SwatchSize` gives the palette swatch, and is why an iOS row is now the twin's 48 pt rather than the 56 the old tile forced (D1, `Packages/SalusUI/Sources/SalusUI/component/SalusSelectableRow.swift:18-33`, `:120-128`, `:196-201`). |
