...
| (d) | `SalusBottomSheet` is a modifier over `.sheet` + `presentationDetents`, not a view; the language picker uses the `.medium` detent alone and the theme picker `[.medium, .large]` — its content (mode tiles + section header + four palette rows) is taller than a medium sheet on every iPhone. The sheet **body scrolls** its caller's content in every case, because a SwiftUI sheet clips where Compose's `ModalBottomSheet` column scrolls. |
...
| (p) | Home's snapshot pager is a **fixed height** — `HomePagerDefaults.height`, `@ScaledMetric`-scaled — because SwiftUI's `TabView(.page)` has no intrinsic height to take from its tallest page the way Compose's `HorizontalPager` does (T6, `Packages/Features/FeatureHome/Sources/FeatureHome/ui/HomeSnapshotPager.swift:60`, `:225-231`). |
